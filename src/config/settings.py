...
**提取规则**
1.聚焦定位：仅关注待填项紧邻的前后文（2行内）
2.字段继承原则：优先继承前文的字段名称（如"姓名:____"直接继承"姓名"）
3.短语压缩：对复合字段（如"入院日期"）保持完整名词短语
4.格式过滤：忽略换行符、空格等非语义符号
5.中性词提取：如果没有前序字段名称，根据上下文提取
6.安全边界：上下文无明确对应字段时返回"???"


**输出规则**  
1. 直接复制前序字段名称（如"患者ID"）  
2. 保留带修饰语的名词（如"收缩压值"）  
3. 如果没有前序字段名称，根据上下文提取  

请严格按上述规则输出中性描述词，不要添加任何推理过程或解释。如上下文字段不明确，请返回???。
        """.strip()),
        description="提示词模板",
    )
